...
This chapter has been accepted as demo paper: 
Tabuenca, B., B\"orner, D., Kalz, M., \& Specht, M. (2015). User-modelled ambient feedback for self-regulated learning. \textit{In Proceedings of the 10th European Conference on Technology Enhanced Learning. Design for teaching and learning in a networked world}. Toledo, Spain. In Press
...
The NFC-LearnTracker lets the user configure which feedback signal\footnote{Feedback Cube effects in video: \\ https://sites.google.com/site/lifelonglearninghubproject/home/feedback-cube} fits better each one of the events listed below. Hereby we present the events supported and their default set-up (Table \ref{tbl:nfceco_table1}):
...
On approach to the beacon                                                                                    	& \begin{tabular}[c]{@{}l@{}}The user with BLE-enabled\\ smartphone approaches to the\\ beacon.\end{tabular}                & \begin{tabular}[c]{@{}l@{}}Summarize remaining tasks. The cube\\ lights a pie chart indicating the\\ distribution of pending tasks/time (Figure \ref{fig:nfceco_6}) \end{tabular}        \\ \hline
On check-in tapping the NFC tag                                                                              	& \begin{tabular}[c]{@{}l@{}}The user taps on the\\ NFC tag every time he starts a learning activity.\end{tabular} 	 		& \begin{tabular}[c]{@{}l@{}}Start! As the user has tapped the\\ blue tag bound to the tablet\\ (Figure \ref{fig:nfceco_2}), the cube\\ lights the blue colour (See \ref{fig:nfceco_1})\\ to indicate he is working\\ on the learning goal configured\\ with the blue in the NFC-LearnTracker (Figure \ref{fig:nfceco_3})\end{tabular}       \\ \hline
