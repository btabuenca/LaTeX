...
In \cite{Andersen2013} key locations at the campus like cafés, lecture halls and meeting points, are tagged with NFC tags to check-in when entering these locations. This action is registered in a social network that enabled a user to share his location with friends, via the "check in" \footnote{Foursquare social network. https://foursquare.com} so friends are aware when a person arrives  to a location (i.e. check-in at the canteen during the coffee break). A smart classroom system integrates NFC technology to automate attendance management, locate students, and provide real-time feedback via personal response systems \citep{Shen2014}.
...
\item Visualization. The screens of mobile devices, tablets, or even laptops are not big enough to display videos. Indeed, videos should be streamed with a quality that guarantees the user can smoothly accomplish objectives of the learning resource with the least visual overload.
\end{enumerate}
...
In future research, we will scaffold ecologies to provide effective feedback services for learning (using ambient displays, recommendations, analytics) and to foster meta-cognitive skills \cite{Tabuenca2014}. 
