...
\begin{itemize}
\item Hypothesis 2: The intermediate measure M2 is significantly higher in contrast to the initial measure in M0, than the final measure M3 in contrast to the initial measure in M0.
\end{itemize}
...
M0                                                                                      & X                   	& X                 	& X                   	& X                 \\ \hline
M2                                                                                      & (t=-2.52, p=.01)*   & (t=-1.47, p=.15)  & -        & -                 \\ \hline
M3                                                                                      & -                   	& -                 	& (t=-2.52, p=.07)    & (t=-1.86, p=.07)   \\ \thickhline

\end{tabular}
\end{table}

The differences in the contrasts confirm significant differences in the variances from the initial measure M0 to M2 in TM (p=.01). Nevertheless, the variances in TM and TP are not significant in M3 in contrast to M0 (p=.07 in both TM and TP). Figure \ref{fig:sload_9} shows that the differences in the TM mean contrasts to the initial measure M0 are slightly higher in M2 (mean(M2) - mean(M0) = .77) than in M3 (mean(M3) - mean(M0) = .72), and consequently confirming our hypothesis.

\begin{center}
\begin{figure}[ht]
\centering
	\subfloat[Contrast 1. Notifications pushed at fixed time of the day]{
		\includegraphics[width=0.5\linewidth]{img/studyload_fig9a}
		\label{fig:sload_9a}
	}
	\subfloat[Contrast 2. Notifications pushed on random basis in the time of the day]{
		\includegraphics[width=0.5\linewidth]{img/studyload_fig9b}
		\label{fig:sload_9b}
	}	
...
Measure                                                                                & TM                  & TP                & TM                  & TP                \\ \hline
M0                                                                                      & X                   & X                 & X                   & X                 \\ \hline
M1                                                                                      & (t=-2.14, p=.04)*   & (t=-1.19, p=.24)  & -                   & -                 \\ \hline
M2                                                                                      & -                   & -                 & (t=-2.52, p=.01)*   & (t=-.47, p=.15)   \\ \thickhline
\end{tabular}
\end{table}

The differences in the contrasts for TM confirm significant variances in M2 (p=.01) and M1 (p=.04). The differences in the contrasts do not confirm significant variances for TP. Figure \ref{fig:sload_14} shows that the differences in the mean contrasts to the initial measure M0 are slightly higher in M2 (mean(M2) - mean(M0) = .77) than in M1 (mean(M1) - mean(M0) = .70), and consequently confirming our hypothesis.

\begin{center}
\begin{figure}[ht]
\centering
	\subfloat[Contrast 1. Notifications containing generic tips]{
		\includegraphics[width=0.5\linewidth]{img/studyload_fig14a}
		\label{fig:sload_14a}
	}
	\subfloat[Contrast 2. Notifications containing learning analytics]{
		\includegraphics[width=0.5\linewidth]{img/studyload_fig14b}
		\label{fig:sload_14b}
	}		
      \caption{Boxplots with measure of means in “time management” subscale. 5) Strongly disagree; 4) Disagree; 3) Neutral; 2) Agree; 1) Strongly agree;}
      \label{fig:sload_14}
